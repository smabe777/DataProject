...
public class DbService {
	...
	public List<Day> findDayByPersonAndByDate(Person person, String date) {
		...
		TypedQuery<Day> query = entityManager.createQuery("select d from Day d where d.person = ?1 and d.date = ?2", Day.class);
		query.setParameter(1, person);
	...
	public List<Person> findByLastname(String lastname, int page, int pageSize) {
		...
		TypedQuery<Person> query = entityManager.createQuery("select p from Person p where p.lastname = ?1", Person.class);
